...
test('DOM: ten seasons with service choices, records and review render cleanly', () => {
  ...
  try {
    ...
    // Negotiations: default offers, then accept no counter-offers.
    action(d, 'offer-confirm');
    if (d.querySelector('[data-action=counter-confirm]')) action(d, 'counter-confirm');
