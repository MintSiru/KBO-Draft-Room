...
`examples/demo-five-seasons.json`을 게임의 ‘불러오기’로 열면 5년을 다 마친 진행 상태를 바로 볼 수 있습니다.
...
- 저장에는 게임의 입력(설정·시드·내 지명·기자회견 답·진행 시즌 수)만 들어갑니다. 5년 진행 파일도 1KB가 안 됩니다. 불러올 때 나머지를 다시 계산합니다.
- 저장에는 시뮬레이션 버전(`sim`)이 기록됩니다. 규칙이 바뀐 버전에서 이전 저장을 열면 다른 결과로 이어 가지 않고, 이어 할 수 없다고 알려 줍니다.
- v0.6.0에서 만든 저장도 열립니다. V0.5 저장은 능력치 체계가 달라 호환되지 않습니다.
