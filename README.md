...
4. **육성선수 계약**: 지명되지 않은 선수 중 최대 5명. 첫 시즌엔 1군 주전이 될 수 없습니다. 다른 구단도 3–5명씩 계약합니다.
5. **입단**: 단장 기자회견 약속과 입단 소감.
6. **10시즌**: 보직·기록·성장·부상·트레이드·방출·은퇴가 해마다 이어집니다.
...
7. **평가**: 10개 구단의 드래프트를 WAR·주전 배출·기량 발전으로 비교합니다. 중간 평가는 언제든 볼 수 있습니다.
...
- 저장에는 게임의 입력(설정·시드·내 지명·육성선수·기자회견 답·병역 결정·진행 시즌 수)만 들어갑니다. 10시즌 진행 파일도 1KB가 안 됩니다. 불러올 때 나머지를 다시 계산합니다.
- 저장에는 시뮬레이션 버전(`sim`)이 기록됩니다. 규칙이 바뀐 버전에서 이전 저장을 열면 다른 결과로 이어 가지 않고, 이어 할 수 없다고 알려 줍니다.
- 0.8.0은 시즌 규칙이 바뀌어(`sim` 0.8) 0.7.x 이전 저장을 이어 할 수 없습니다. 이전 자동 저장은 지우지 않고 따로 보관합니다.
