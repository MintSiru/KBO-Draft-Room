...
function makeBiography(r,region,pathway){
 ...
 if(pathway==='대졸'){current=choose(Cat.institutions.filter(s=>s.kind==='college'),r);hsGrad=ENTRY_YEAR-4;}
 ...
 if(pathway==='대졸')history.push({institutionId:current.id,name:current.name,kind:current.kind,region:current.region,tier:current.tier,start:iso(hsGrad,3),end:iso(ENTRY_YEAR,2,28),status:'졸업 예정',note:'게임에서는 4년제 과정으로 단순화합니다.'});
 ...
 return {birthday,age:ageAt(birthday),birthRegion,birthplace,cohort:birthYear,highSchoolId:hs.id,highSchoolName:hs.name,highSchoolRegion:hs.region,highSchoolGradYear:hsGrad,currentInstitutionId:current.id,currentInstitution:current,school:current.name,schoolTier:current.tier,history,pathText:history.map(h=>h.name).join(' → '),qualification:high?'고교 졸업 예정':pathway==='대졸'?'대학 졸업 예정':pathway==='독립구단'?'독립구단 지원':'해외 대학 졸업',regionalEligible:high,regionalRegion:high?hs.region:null,regionalReason:high?(hs.kind==='hs-club'?'고교 연령 클럽도 고졸 지원자로 허용하는 게임용 규칙입니다.':'현재 고교 졸업 예정자로서 소속 지역 기준으로 판정합니다.'):'과거 고교와 출생 지역에 관계없이 현재 지원 구분상 지역 1차 지명에서 제외합니다.'};
}
