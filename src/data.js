...
function generatePool(seed){
 ...
 for(let i=0;i<200;i++){
  ...
  let control=round(38+r()*40+(type===1&&pitcher?10:0)+(schoolStyle==='투수 육성'&&pitcher?2:0)),power=round(37+r()*38+(type===1&&!pitcher?13:0)+(schoolStyle==='타격 중심'&&!pitcher?2:0));
  const speed=round(38+r()*40+((role==='OF'&&[0,4].includes(type))||(role==='IF'&&type===2)?10:0)+(schoolStyle==='기동력 야구'?2:0));
  const defense=round(40+r()*39+((role==='C'&&[0,3].includes(type))||(!pitcher&&type===0)?9:0)+(schoolStyle==='수비 기본기'?2:0));
  const velocity=round(clamp(132+ready*.19+r()*7+(type===0?4:0),136,156));
  ...
  players.push({id:'p'+String(i+1).padStart(3,'0'),...identity,...bio,region,pathway,school,schoolStyle,schoolTournament,role,type,archetype:a[0],strength:Ko.formal(a[1]),weakness:Ko.formal(a[2]),focus:a[3],age,height,weight,throwHand,batHand,ready,trueReady,upside,scoutCeiling,publicScore,velocity,control,power,speed,defense,awards,record,risk:.05+r()*.11,personality:pick(PERSONALITIES,r),favoriteTeam:Math.floor(r()*10),lateDevelopment:r()<.13,confidence:record.games>=25?'보통':'관찰 표본 적음'});
 }
...
function amateurRecord(p,r){
 if(['SP','RP'].includes(p.role)){
  ...
  const er=Math.max(2,round(ip*(6.5-p.ready*.069+normal(r)*.8)/9));
 ...
 const hr=Math.min(hits,round(ab*Math.max(.001,(p.power-38)/1300)*r())),doubles=Math.min(hits-hr,round(hits*.18)),triples=Math.min(hits-hr-doubles,Math.floor(r()*3));
 const avg=round(hits/ab,3),ops=round((hits+bb)/(ab+bb)+(hits+doubles+2*triples+3*hr)/ab,3);
 return {kind:'hitter',games,ab,pa:ab+bb,hits,bb,hr,doubles,triples,avg,ops,sb:round((p.speed-25)*games/140*r()),rbi:round((hits*.3+hr)*p.teamSupport),runs:round((hits+bb)*.4*p.teamSupport)};
}
const api={catalog:Cat.institutions,schoolHonors,bio:Bio,ko:Ko,names:Names,ROLES,REGIONS,SCHOOLS,SCHOOL_STYLES,COLLEGES,INDEPENDENTS,OVERSEAS,PERSONALITIES,ARCHETYPES,hash,rng,clamp,round,mean,pick,shuffle,normal,generatePool};
